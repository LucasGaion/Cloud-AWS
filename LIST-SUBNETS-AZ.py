...
for contas in accounts:
    ...
    for region in regions:
        try:
            ...
            subs = ec2_client.describe_subnets()

            for sub in subs['Subnets']:
                subnet_name = next((tag['Value'] for tag in sub.get('Tags', []) if tag['Key'] == 'Name'), 'N/A')
                worksheet.append([
                    contas['Name'],
                    region,
                    subnet_name,
...
data = date.today()
sheetSUB = f'LIST SUBNETS - AZ UNICA - {data}.xlsx'
workbook.save(filename=sheetSUB)
print(f'Salvo com sucesso: {sheetSUB}')
