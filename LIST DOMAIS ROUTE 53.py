...
for contas in accounts:
    ...
    for region in regions:
        try:
            ...
            route53domains = credentials.client('route53domains', region_name=region)
            route53domains = route53domains.list_domains()

            for domains in route53domains['Domains']:
                worksheet.append([
                    contas['Name'],
                    region,
                    domains['DomainName'],
...
data = date.today()
sheetROUTE53 = f'LIST DOMAINS ROUTE53- {data}.xlsx'
workbook.save(filename=sheetROUTE53)
print(f'Salvo com sucesso: {sheetROUTE53}')
